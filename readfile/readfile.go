...
func ReadPar(par *tp.Par) {
	...
	par.SmoothForwardPercent = ParsetoInt(parTemp["PERCENT_MOVED_FORWARD"])
	par.SmoothBackworkPercent = ParsetoInt(parTemp["PERCENT_MOVED_BACKWARD"])
	par.SmoothIter = ParsetoInt(parTemp["SMOOTHING_ITERATIONS"])
	...
	for scanner.Scan() {
		if i == 0 { //first record, field names
			...
			if i > 1 && (ts != tsCurr || iNodeID != iNodeIDCurr) { //diffrent origin node or time step
				if iNodeIDCurr != Record[1] { //exclude intra zonal trips
					...
					for col, element := range par.VehClass {
						...
						if len(trips) > 0 {
							...
							TaskQueue <- &task
							//fmt.Printf("task queue length %+v\n", len(TaskQueue))
						}
					}
				}
				jNodes = nil
